...
BACKBONE_MAP: Dict[str, type] = {
    ...
    "BagelSiglip": BagelSiglipFeatureExtractor  # Uses default path from models/Bagel
}
...
def main(cfg: MainConfig):
    ...
    # Initialize wandb using shared utility (returns True if enabled)
    wandb_enabled = setup_wandb(cfg, tags=["image_generation"])
    if wandb_enabled:
    ...
    for i, ((image_org, _, path_org), (image_tgt, _, path_tgt)) in enumerate(
        ...
        print(f"\n=== LOOP ITERATION {i} ===")
        print(f"image_org shape: {image_org.shape}")
        print(f"image_tgt shape: {image_tgt.shape}")
        print(f"path_org: {path_org}")
...
def attack_imgpair(
    ...
    # Save images
    print(f">>> About to save {len(path_org)} images")
    for path_idx in range(len(path_org)):
        print(f">>> Saving image {path_idx}")
        folder, name = (
...
def log_metrics(pbar, metrics, img_index, epoch=None):
    """
    Log metrics to progress bar and optionally to wandb.
    ...
    # Log to wandb only if it's available and initialized
