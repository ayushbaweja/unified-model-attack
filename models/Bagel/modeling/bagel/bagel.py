...
class Bagel(PreTrainedModel):
    ...
    def forward(
        ...
        # Embed text tokens using the model's embedding layer (token id -> high dim embedding)
        packed_text_embedding = self.language_model.model.embed_tokens(packed_text_ids)

        # Create a zero-initialized tensor for the full sequence including text and image tokens
        packed_sequence = packed_text_embedding.new_zeros(size=(sequence_length, self.hidden_size))

        # Text embeddings are placed at their positions in the sequence
        ...
        # Use ViT if visual understanding is enabled
        if self.config.visual_und:
            cu_seqlens = torch.nn.functional.pad(torch.cumsum(vit_token_seqlens, dim=0), (1, 0))
            cu_seqlens = cu_seqlens.to(torch.int32)
            max_seqlen = torch.max(vit_token_seqlens).item()

            # Processes image patches through the ViT model to get embeddings
            ...
            # Features are projected to match language model's hidden dim
            packed_vit_token_embed = self.connector(packed_vit_token_embed)
            # Position embeddings are added to the image embeddings
            vit_token_pos_emb = self.vit_pos_embed(packed_vit_position_ids)
            packed_vit_token_embed = packed_vit_token_embed + vit_token_pos_emb
            # Added to designated positions in the sequence
            packed_sequence[packed_vit_token_indexes] = packed_vit_token_embed

        # Use VAE if visual generation is enabled
        if self.config.visual_gen:
            p = self.latent_patch_size
            packed_latent = []
            for latent, (h, w) in zip(padded_latent, patchified_vae_latent_shapes):
                # Patchifying latent features so that discrete patches can be processed as tokens
                latent = latent[:, :h * p, :w * p].reshape(self.latent_channel, h, p, w, p)
                latent = torch.einsum("chpwq->hwpqc", latent).reshape(-1, p * p * self.latent_channel)
                packed_latent.append(latent)
            packed_latent_clean = torch.cat(packed_latent, dim=0)

            # Noise is added to clean concatenated latents following timestep schedule
            # Flow Matching
            ...
            # Projecting to language model space using vae2llm with timestep and position embedding
